...
func TestListWrongId(t *testing.T) {
	...
	//building request
	request, err := http.NewRequest(http.MethodGet, "http://127.0.0.1:8080/api/v1/listDoctor?name=123", nil)
	if err != nil {
		log.Printf("error occured in test case during making request. Error =  %s", err)
		t.Fail()
	}
...
func TestListNilId(t *testing.T) {
	...
	//building request
	request, err := http.NewRequest(http.MethodGet, "http://127.0.0.1:8080/api/v1/listDoctor", nil)
	if err != nil {
		log.Printf("error occured in test case during making request. Error =  %s", err)
		t.Fail()
	}
